...
type ToggleProps = TogglePrimitiveProps & ToggleItemProps & VariantProps<typeof toggleVariants>;
    ...
    <ToggleProvider value={{ isPressed, setIsPressed, disabled }}>
      ...
      >
        <motion.button data-slot="toggle" whileTap={{ scale: 0.95 }} className="relative">
          ...
          <ToggleItem className={cn(toggleVariants({ variant, size, className }))} {...props} />
        </motion.button>
      </TogglePrimitive.Root>
    </ToggleProvider>
  );
